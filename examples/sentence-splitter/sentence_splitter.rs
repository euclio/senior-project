...
fn split_sentences(input: String) {
    ...
    for (mut pos, _) in chars.iter().enumerate().filter(
        ...
        // Periods followed by a digit with no intervening whitespaces are not sentence boundaries.
        if pos + 1 < input.len() && chars[pos + 1].is_digit(10) {
            continue
        }

        // Periods folled by whitespace and then an upper case letter, but preceded by any of a
        // short list of titles are not sentence boundaries.
        let titles: HashSet<&'static str> =
                ["Mr.", "Mrs.", "Dr.", "Jr."].iter().map(|&x| x).collect();
        if pos >= 2 {
            let two_previous_chars: String = chars[pos-2..pos+1].iter().map(|&x| x).collect();
            if titles.contains(&two_previous_chars[..]) {
                continue
            }
        }
        if pos >= 3 {
            let three_previous_chars: String = chars[pos-3..pos+1].iter().map(|&x| x).collect();
            if titles.contains(&three_previous_chars[..]) {
                continue
            }
        }

        // Periods internal to a sentence of letter with no adjacent whitespace are not sentence
        // boundaries (e.g., websites).
        if pos + 1 < input.len() &&
        ...
        // Do nothing, we are just trying to consume any periods that are next to each other.
